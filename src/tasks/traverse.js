...
var basedir = process.cwd(),
    webgme = require(basedir+'/webgme'),
    FS = require('fs'),
    path = require('path'),
    Q = require('q'),
    STORAGE_CONSTANTS = webgme.requirejs('common/storage/constants'),
    gmeConfig = require(basedir+'/config'),
    logger = webgme.Logger.create('gme:bin:import', gmeConfig.bin.log),
    Core = webgme.core,
    FSBlobClient = require(basedir+'/src/server/middleware/blob/BlobClientWithFSBackend'),
...
main = function (argv) {
    ...
    webgme.getGmeAuth(gmeConfig)
        ...
        .then(function (root) {
            ...
            globalMeasurements.executionTime = new Date().getTime();
            return Q.nfcall(core.traverse, root, {}, function (node, next) {
                ...
                if(memory > globalMeasurements.memoryUsage){
                    globalMeasurements.memoryUsage = memory;
                }
                globalMeasurements.numberOfNodes+=1;

                if(core.isMetaNode(node)){
                    globalMeasurements.sizeOfMeta+=1;
                }
        ...
        .then(function () {
            ...
            globalMeasurements.executionTime = new Date().getTime() - globalMeasurements.executionTime;
            FS.writeFileSync(basedir+'/globals_'+program.projectName+'.json',JSON.stringify(globalMeasurements,null,2));
            finishUp(null);
        })
        .catch(finishUp);
    return mainDeferred.promise;
